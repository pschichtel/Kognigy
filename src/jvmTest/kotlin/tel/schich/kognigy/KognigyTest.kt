...
class KognigyTest {
    ...
    private inline fun withKognigy(
        ...
        val kognigy: Kognigy = defaultKognigy()
    ...
    ) = withKognigy(sessionId, userId) { kognigy, session ->
    ...
    /**
     * The endpoint used here should be backed by a flow that simply reproduces a
     * response for each input it gets.
     */
    @EnabledIfEnvironmentVariables(
        EnabledIfEnvironmentVariable(named = ENDPOINT_URL_ENV, matches = ".*"),
